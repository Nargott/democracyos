...
exports.doSync = function doSync(data, meta, callback) {
  ...
  connection.connect((err) => {
    ...
    let query = 'SELECT * FROM ' + config.mysql.database + '.members WHERE is_active = 1 LIMIT ' + limit +
        ' OFFSET ' + offset
    let answer = []
              ...
              User.findByEmail(profile.email, function (err, user) {
                ...
                if (user) {
                  ...
                  User.register(userReg, profile.password, function( err, user) {
                    ...
                    let query = 'UPDATE `members` SET `is_in_vote` = 1 WHERE `members`.`id` = '+ profile.id +';'
                    connection.query(query, (err, rows) => {
                      if (err) {
                        callback(err, false)
                        return
