...
exports.doSync = function doSync (profile, meta, callback) {
  ...
  connection.connect((err) => {
    if (err) throw err
    log('MySQL connected!')
    connection.query('SELECT * FROM elegion.members WHERE is_active = 1 LIMIT 10', (err, rows) => {
      if (err) throw err
      ...
      rows.forEach((row) => {
        ...
        let data = {
          last_name: decrypt(row.last_name, key, iv),
          first_name: decrypt(row.first_name, key, iv),
          email: decrypt(row.email, key, iv),
          phone: decrypt(row.phone, key, iv)
        }
